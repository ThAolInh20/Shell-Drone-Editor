...
export function setupBaseEditorUI(state, director, options = {}) {
  ...
  leftContainer.innerHTML = `
    <h2 style="margin: 0 0 15px 0; font-size: 18px; color: #fff;">${title}</h2>
    ${subtitle ? `<p style="color: #888; font-size: 12px; margin-top: -10px;">${subtitle}</p>` : ''}
  ...
  // Bind Events & Logic for individual panels
  setupPanels();
}

function makePanelsCollapsible(shouldCollapse) {
  ...
  sections.forEach(section => {
    ...
    let isCollapsed = shouldCollapse(titleText);
